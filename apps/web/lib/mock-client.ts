...
const STAGE_MESSAGE: Record<PaymentStatus, (p: PaymentRecord) => string> = {
  ...
  OnRamped: (p) => `KES converted to ${toDecimal(p.usdcMinor ?? 0, "USDC").toFixed(2)} USDT (mock)`,
  Bridging: (p) => `USDT transfer submitted (mock) tx=${(p.walletTxHash ?? "").slice(0, 14)}…`,
  Bridged: (p) => `USDT confirmed on-chain (att=${p.attestationId ?? ""})`,
  PayingOut: (p) =>
    `HK off-ramp: USDT → HK$${hkdFromUsdMinor(p.usdMinor)} @ ${HK_FX_RATE.toFixed(2)} (stub); paying supplier`,
